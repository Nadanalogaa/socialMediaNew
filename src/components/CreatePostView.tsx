...
export const CreatePostView: React.FC<CreatePostViewProps> = ({ connections, onPostPublished }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files) {
            const newAssets: MediaAsset[] = Array.from(files).map(file => ({
                ...
                description: '',
                hashtags: [],
                platforms: [PlatformEnum.Facebook, PlatformEnum.Instagram],
                status: 'idle',
            }));
    ...
    return (
        <div className="max-w-7xl mx-auto animate-fade-in space-y-8">
            ...
            {assets.map((asset) => {
                const isPublishDisabled = asset.status === 'publishing' || asset.status === 'published' || asset.platforms.length === 0;
                                        ...
                                        {Object.values(PlatformEnum).map(p => (
                                            <button key={p}
                                                onClick={() => {
                                                    ...
                                                    updateAsset(asset.id, { platforms: newPlatforms, status: 'idle', errorMessage: undefined });
                                                }}
